...
<div class="wrap">
	...
	<table summary="공지사항을 나타낸 표" class="board_list">
		...
		<tr class="notice">
			...
			<td align="center">
			<c:if test="${b.bType eq 1 }">정보공유</c:if>
			<c:if test="${b.bType eq 2 }">무료나눔</c:if>
			<c:if test="${b.bType eq 3 }">베이비시터모집</c:if>
			<c:if test="${b.bType eq 4 }">베이비시터지원</c:if>
			<c:if test="${b.bType eq 5 }">문의사항</c:if>
			<c:if test="${b.bType eq 6 }">공지사항</c:if>
			<c:if test="${b.bType eq 7 }">신고</c:if>
			<c:if test="${b.bType eq 8 }">리뷰</c:if>		
			</td>
			<td align="left">
		...
		<tr align="center" height="20" id="buttonTab">
			<td colspan="6">
				...
				<c:if test="${ pi.currentPage > 1 }">
					<c:url var="before" value="mylikeList.do">
				...
				<c:forEach var="p" begin="${ pi.startPage }" end="${ pi.endPage }">
					...
					<c:if test="${ p ne currentPage }">
						<c:url var="pagination" value="mylikeList.do">
				...
				<c:if test="${ pi.currentPage < pi.maxPage }">
					<c:url var="after" value="mylikeList.do">
						<c:param name="page" value="${ pi.currentPage + 1 }"/>
					</c:url> 
					<a href="${ after }">[다음]</a>
				</c:if>
